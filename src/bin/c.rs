...
fn main() {
    ...
    let sum = a.iter()
        .fold(ModInt1000000007::new(0), |acc, &x| acc + x);

    let sum_of_squared = a.iter()
        .fold(ModInt1000000007::new(0), |acc, &x| acc + x * x);

    let ans_doubled = sum * sum - sum_of_squared;

    let ans = ans_doubled / 2;
